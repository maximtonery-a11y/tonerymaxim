...
test('publikačný rad odmietne duplicitu a vyžaduje presné dvojité potvrdenie',async()=>{const nonce=randomUUID(),name=`TM Search test ${nonce}`,bundle:any={mode:'DRY_RUN_ONLY',approvalFingerprint:'a',campaign:{name,channel:'search',status:'PAUSED'}},first=await enqueueDeployment(`draft-${nonce}`,bundle),second=await enqueueDeployment(`draft-${nonce}`,bundle);assert.equal(first.duplicate,false);assert.equal(second.duplicate,true);await assert.rejects(()=>deploymentAction(first.item.id,'confirm','áno'),/presne znieť/i);const confirmed=await deploymentAction(first.item.id,'confirm',`PAUSED: ${name}`);assert.equal(confirmed.status,'APPROVED_FOR_VALIDATE_ONLY');const validated=await deploymentAction(first.item.id,'validated',undefined,{validateOnly:true});assert.equal(validated.status,'VALIDATED');assert.equal((validated.validation as any).validateOnly,true)});

test('zrušená položka publikačného radu sa dá zaradiť znova, validovaná nie je zmazateľná',async()=>{const nonce=randomUUID(),name=`TM Shopping cancel ${nonce}`,bundle:any={mode:'DRY_RUN_ONLY',approvalFingerprint:'b',campaign:{name,channel:'shopping',status:'PAUSED'}},q=await enqueueDeployment(`draft-${nonce}`,bundle);const cancelled=await deploymentAction(q.item.id,'cancel');assert.equal(cancelled.status,'CANCELLED');const again=await enqueueDeployment(`draft-${nonce}`,bundle);assert.equal(again.duplicate,false);await deploymentAction(again.item.id,'confirm',`PAUSED: ${name}`);await deploymentAction(again.item.id,'validated',undefined,{validateOnly:true});await assert.rejects(()=>deploymentAction(again.item.id,'cancel'),/nemožno/i);const store=await readDeploymentQueue();assert.ok(store.audit.some(x=>x.action==='cancelled'))});
