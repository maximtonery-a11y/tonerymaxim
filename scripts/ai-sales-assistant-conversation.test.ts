...
test('model -> shipping CZ keeps product and adds CZ', async()=>{const {b}=await convo('Mám Brother HL-L2352DW, aký toner?','a posielate to do Česka?'); assert.equal(b.intent,'product_search'); assert.match((b.answer||[]).join(' '),/Českej republiky|Ceskej republiky/i);});
...
test('model -> delivery followup', async()=>{const {b}=await convo('Potrebujem CF283A','a kedy mi príde?'); assert.equal(b.intent,'product_search'); assert.match((b.answer||[]).join(' '),/1–2|1-2|exped/i);});
