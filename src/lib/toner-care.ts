...
async function sendCustomerReminder(customer: WooCustomer, printer: SavedPrinter): Promise<void> {
  const accountUrl = storefrontUrl("/ucet/tlaciarne");
  const productsUrl = storefrontUrl(printer.url || "/produkty", "/produkty");
  const name = customerName(customer);
  const due = formatDate(String(printer.expected_replacement_at || ""));
  const preference = preferenceLabel(printer.preferred_type);

  const text = `Dobrý deň, ${name},\n\nv časti Moje tlačiarne ste si pre tlačiareň ${printer.title} nastavili upozornenie na plánovanú výmenu náplne. Podľa zadaného termínu môže byť výmena potrebná približne ${due}, preto vám túto pripomienku posielame už teraz – približne 3 týždne vopred.\n\nAk chcete mať náplň pripravenú včas, môžete si vhodný ${preference} rovno vybrať a objednať:\n${productsUrl}\n\nUpozornenie môžete upraviť alebo označiť výmenu vo svojom účte:\n${accountUrl}\n\nTermín je orientačný a vychádza z údajov, ktoré ste zadali. Bez vášho potvrdenia objednávky vám nič automaticky neposielame.\n\nToneryMAXIM.sk`;
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.55;color:#061735;max-width:640px;margin:0 auto;padding:24px">
      ...
      <h1 style="font-size:25px;margin:0 0 16px">Pripomíname výmenu náplne, ktorú ste si naplánovali</h1>
      <p>Dobrý deň, ${escapeHtml(name)},</p>
      <p>v časti <strong>Moje tlačiarne</strong> ste si pre tlačiareň <strong>${escapeHtml(printer.title)}</strong> nastavili upozornenie na plánovanú výmenu náplne. Podľa zadaného termínu môže byť výmena potrebná približne <strong>${escapeHtml(due)}</strong>, preto vám túto pripomienku posielame už teraz – približne 3 týždne vopred.</p>
      <p>Ak chcete mať náplň pripravenú včas, môžete si vhodný <strong>${escapeHtml(preference)}</strong> rovno vybrať a objednať.</p>
      <p><a href="${escapeHtml(productsUrl)}" style="display:inline-block;background:#061735;color:#fff;text-decoration:none;padding:12px 18px;border-radius:999px;font-weight:700">Vybrať a objednať vhodnú náplň</a></p>
      <p><a href="${escapeHtml(accountUrl)}" style="color:#066bc9;font-weight:700">Upraviť upozornenie v mojom účte</a></p>
      <p style="padding:14px;border-radius:14px;background:#f3f8fd;color:#51627d;font-size:13px">Termín je orientačný a vychádza z údajov, ktoré ste zadali. Bez vášho potvrdenia objednávky vám nič automaticky neposielame.</p>
      <p>ToneryMAXIM.sk</p>
    </div>`;

  await sendMail({
    to: customer.email,
    subject: `Pripomienka, ktorú ste si nastavili pre ${printer.title} | ToneryMAXIM.sk`,
    text,
    html,
  });
}
