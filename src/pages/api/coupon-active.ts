...
import { getBestAutoCoupon } from "../../lib/coupons";
...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    const coupon = await getBestAutoCoupon(session?.id, cart);
    return new Response(JSON.stringify(coupon || { ok: false, code: "", reason: "Nemáte aktívny kupón použiteľný v tomto košíku." }), {
      status: 200,
      headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store" },
    });
  } catch (error: any) {
    return new Response(JSON.stringify({ ok: false, code: "", reason: error?.message || "Kupón sa nepodarilo načítať." }), {
      status: 500,
      headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store" },
    });
  }
