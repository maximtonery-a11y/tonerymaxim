...
import { publicOrderStatus } from '../../lib/ai-order-status';
...
export const POST: APIRoute = async ({ request, clientAddress }) => {
  ...
  try {
    const candidates = await wooRequest<any[]>('/orders', { query:{ search:orderNumber, per_page:10 } });
    const order = (Array.isArray(candidates) ? candidates : []).find(item => String(item?.number || item?.id) === orderNumber);
