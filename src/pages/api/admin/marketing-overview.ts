...
export const GET:APIRoute=async({request,url,locals})=>{if(!allowed(request,url,locals))return Response.json({ok:false,error:'Unauthorized'},{status:401});const [events,drafts,googleAds]=await Promise.all([readAnalyticsEvents(10000),readMarketingDrafts(),readGoogleAdsStatus((locals as any)?.runtime?.env)]);const learning=aggregateLearning(readLearningEvents());return Response.json({ok:true,generatedAt:new Date().toISOString(),overview:marketingOverview(events,learning),drafts,googleAds},{headers:{'Cache-Control':'no-store'}})};
