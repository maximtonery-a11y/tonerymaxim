...
(function () {
  ...
  function renderTypeQuestion(action){
    ...
    const variantText=count=>count===1?'Máme 1 produktový variant':count>=2&&count<=4?`Máme ${count} produktové varianty`:`Máme ${count} produktových variantov`;
    const cards=options.map(type=>{const count=Number(counts[type]||0),label=labels[type]||type;return `<article class="tm-ai-type-card is-${type}"><div class="tm-ai-type-card__head"><b>${label}</b>${type==='compatible'?'<em>Odporúčame</em>':''}</div><small class="tm-ai-type-card__count">${variantText(count)}</small><p>${reason[type]||''}</p><button type="button" data-ai-type-more="${type}">Zobraziť ${label.toLocaleLowerCase('sk-SK')}</button></article>`}).join('');
    const a=addMessage('bot',`<div class="tm-ai-type-compare"><div class="tm-ai-guided-title"><b>Vyberte typ náplne</b><span>Potom zobrazím konkrétne varianty, ceny a dostupnosť.</span></div><div class="tm-ai-type-grid">${cards}</div>${options.includes('compatible')?'<div class="tm-ai-type-benefit"><b>Zľava na kompatibilné:</b> pri 2–3 ks 10 % · pri 4 a viac ks 25 %</div>':''}</div>`);
  ...
  function renderCommerceResults(data){
    ...
    const missingRequestedTypes=Array.isArray(data?.presentation?.missingRequestedTypes)?data.presentation.missingRequestedTypes:[];
    const missingTypeText=missingRequestedTypes.length?`<p class="tm-ai-set-missing">${missingRequestedTypes.map(type=>type==='compatible'?'Kompatibilný typ':type==='original'?'Originálny typ':'Renovovaný typ').join(' a ')} momentálne v katalógu nemáme.</p>`:'';
    addMessage('bot',`<div class="tm-ai-offer-summary"><b>Overil som aktuálnu ponuku pre ${query}.</b><p>${available.length?`V ponuke máme <strong>${suitableProductsText(available.length)}</strong>. Produkty skladom ${dispatchSentence()}.`:'Momentálne nemáme vhodný produkt, ktorý je možné objednať.'}${unavailable.length?` V ponuke máme aj <strong>${unavailableProductsText(unavailable.length)}</strong>; ich dostupnosť vám vieme zistiť.`:''}</p>${missingTypeText}<div class="tm-ai-summary-actions"><a href="${escapeHtml(webResultsUrl(all))}">Zobraziť všetky na webe</a></div></div>`,{scroll:false});
    ...
    if(colorPrinter){
      ...
      if(sets.length||singleGroups.length){
        ...
        const requestedOfferTypes=Array.isArray(data?.presentation?.requestedTypes)&&data.presentation.requestedTypes.length?data.presentation.requestedTypes:typeOrder;
        const offerSections=typeOrder.map(type=>{const setCards=setCardsByType.get(type)||'',singleSection=singleSectionsByType.get(type)||'',wasRequested=requestedOfferTypes.includes(type);if(!setCards&&!singleSection&&!(setIntent&&wasRequested))return'';const typeLabel=type==='compatible'?'Kompatibilná sada':type==='original'?'Originálna sada':'Renovovaná sada';const missingSet=setIntent&&!setCards&&wasRequested?`<p class="tm-ai-set-missing">${typeLabel} ako jeden kompletný katalógový produkt momentálne nie je v katalógu.</p>`:'';return `<section class="tm-ai-offer-section is-${type}"><h4>${typeLabel}</h4>${setCards?`<div class="tm-ai-set-list">${setCards}</div>`:''}${missingSet}${setIntent?'':singleSection}</section>`}).join('');
        const offerProductNoun=all.every(product=>isInkOffer([product]))?'atramentové náplne':'tonery';
        const hiddenSingles=setIntent&&singleGroups.length?`<button type="button" class="tm-ai-singles-toggle" data-ai-show-singles>Zobraziť aj jednotlivé ${offerProductNoun}</button><div class="tm-ai-singles-panel" data-ai-singles-panel hidden>${typeOrder.map(type=>singleSectionsByType.get(type)||'').join('')}</div>`:'';
        const unavailableCards=unavailable.slice(0,4).map((p,i)=>productOfferCard(p,i,true)).join('');
        const box=addMessage('bot',`<div class="tm-ai-color-offer tm-ai-guided-offer"><div class="tm-ai-guided-title"><b>${setIntent?'Kompletné CMYK sady':'Vyberte spôsob nákupu'}</b><span>${setIntent?`Zobrazujeme iba skutočné katalógové sady. Jednotlivé ${offerProductNoun} sú dostupné osobitne.`:'Najskôr zobrazujeme kompatibilné, potom originálne a renovované možnosti.'}</span></div>${offerSections}${hiddenSingles}${unavailableCards?`<section class="tm-ai-offer-section is-unavailable"><h4>V ponuke, momentálne nie je skladom</h4><div class="tm-ai-shop-products">${unavailableCards}</div></section>`:''}<div class="tm-ai-discovery__footer"><a href="${escapeHtml(webResultsUrl(all))}">Zobraziť kompletnú ponuku na webe →</a></div></div>`);
  ...
  quick.addEventListener('click', (event) => {
    ...
    if (!button) return;
    input.value = '';
    const question=button.dataset.aiPrompt || button.textContent.trim();
    // Stav objednávky je transakčná akcia, nie voľná poradenská otázka.
    // Kliknutie preto nesmie závisieť iba od jazykového rozpoznania textu.
    const buttonLabel=String(button.textContent||'').toLocaleLowerCase('sk-SK').normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();
    const forceOrderStatus=buttonLabel==='stav objednavky';
    unifiedAsk(question,{forceOrderStatus});
  });
